  ...
  if (aLocation.length) {
    ...
    if (aLocation[0].games.includes(req.body.gameToDelete)) {
      ...
      if (aLocation[0].games.length > 0) {
        ...
        aLocation[0]
          .save()
          .then(() => console.log(`Location ${req.body.name} deleted game ${req.body.gameToDelete}`))
          .then(() => res.status(200).json({Location: req.body.name, GameDeleted: req.body.gameToDelete}))
          .catch((err) =>
            res.status(400).json({ Error: err, Location: req.body.name })
          );
      } 
